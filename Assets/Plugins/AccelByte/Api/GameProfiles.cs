    ...
    public class GameProfiles
    {
        ...
        public void BatchGetGameProfiles(ICollection<string> userIds, ResultCallback<UserGameProfiles[]> callback)
        {
            ...
            this.taskDispatcher.Start(
                Task.Retry(
                    cb => this.api.BatchGetGameProfiles(this.user.Namespace, userIds,
                        this.user.AccessToken, result => cb(result)),
        ...
        public void GetAllGameProfiles(ResultCallback<GameProfile[]> callback)
        {
            ...
            this.taskDispatcher.Start(
                Task.Retry(
                    cb => this.api.GetAllGameProfiles(this.user.Namespace, this.user.UserId, 
                        this.user.AccessToken, result => cb(result)),
        ...
        public void GetGameProfile(string profileId, ResultCallback<GameProfile> callback)
        {
            ...
            this.taskDispatcher.Start(
                Task.Retry(
                    cb => this.api.GetGameProfile(this.user.Namespace, this.user.UserId,
                        this.user.AccessToken, profileId, result => cb(result)),
                    result => this.coroutineRunner.Run(() => callback((Result<GameProfile>)result)),
                    this.user));
        }
        ...
        public void UpdateGameProfile(GameProfile gameProfile, ResultCallback<GameProfile> callback)
        {
            ...
            this.taskDispatcher.Start(
                Task.Retry(
                    cb => this.api.UpdateGameProfile(this.user.Namespace, this.user.UserId,
                        this.user.AccessToken, gameProfile.profileId, gameProfile, result => cb(result)),
                    result => this.coroutineRunner.Run(() => callback((Result<GameProfile>)result)),
                    this.user));
        }

        public void UpdateGameProfile(string profileId, GameProfileRequest gameProfile, ResultCallback<GameProfile> callback)
        {
            if (!this.user.IsLoggedIn)
            {
                callback.Try(Result<GameProfile>.CreateError(ErrorCode.IsNotLoggedIn, "User is not logged in"));
            ...
            this.taskDispatcher.Start(
                Task.Retry(
                    cb => this.api.UpdateGameProfile(this.user.Namespace, this.user.UserId, 
                        this.user.AccessToken, profileId, gameProfile, result => cb(result)),
                    result => this.coroutineRunner.Run(() => callback((Result<GameProfile>) result)),
                    this.user));
        }
        ...
        public void DeleteGameProfile(string profileId, ResultCallback callback)
        {
            ...
            this.taskDispatcher.Start(
                Task.Retry(
                    cb => this.api.DeleteGameProfile(this.user.Namespace, this.user.UserId, 
                        this.user.AccessToken, profileId, result => cb(result)),
                    result => this.coroutineRunner.Run(() => callback((Result) result)),
                    this.user));
        }

        public void GetGameProfileAttribute(string profileId, string attributeName, ResultCallback<GameProfileAttribute> callback)
        {
            if (!this.user.IsLoggedIn)
            {
                callback.Try(Result<GameProfileAttribute>.CreateError(ErrorCode.IsNotLoggedIn, "User is not logged in"));
                return;
            }

            this.taskDispatcher.Start(
                Task.Retry(
                    cb => this.api.GetGameProfileAtrribute(this.user.Namespace, this.user.UserId,
                        this.user.AccessToken, profileId, attributeName, result => cb(result)),
