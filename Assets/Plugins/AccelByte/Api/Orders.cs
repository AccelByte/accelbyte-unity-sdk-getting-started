    ...
    public class Orders
    {
        ...
        public void CreateOrder(OrderRequest orderRequest, ResultCallback<OrderInfo> callback)
        {
            ...
            this.taskDispatcher.Start(
                Task.Retry(
                    cb => this.api.CreateOrder(this.user.Namespace, this.user.UserId,
                        this.user.AccessToken, orderRequest, result => cb(result)),
        ...
        public void GetUserOrder(string orderNo, ResultCallback<OrderInfo> callback)
        {
            ...
            this.taskDispatcher.Start(
                Task.Retry(
                    cb => this.api.GetUserOrder(this.user.Namespace, this.user.UserId, 
                        this.user.AccessToken, orderNo, result => cb(result)),
                    result => this.coroutineRunner.Run(() => callback((Result<OrderInfo>) result)),
                    this.user
                )
            );
        ...
        public void GetUserOrders(uint startPage, uint size, ResultCallback<PagedOrderInfo> callback)
        {
            ...
            this.taskDispatcher.Start(
                Task.Retry(
                    cb => this.api.GetUserOrders(this.user.Namespace, this.user.UserId, 
                        this.user.AccessToken, startPage, size, result => cb(result)),
        ...
        public void GetUserOrderHistory(string orderNo, ResultCallback<OrderHistoryInfo[]> callback)
        {
            ...
            this.taskDispatcher.Start(
                Task.Retry(
                    cb => this.api.GetUserOrderHistory(this.user.Namespace, this.user.UserId, 
                        this.user.AccessToken, orderNo, result => cb(result)),
