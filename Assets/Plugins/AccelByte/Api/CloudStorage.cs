    ...
    public class CloudStorage
    {
        ...
        public void GetAllSlots(ResultCallback<Slot[]> callback)
        {
            ...
            this.taskDispatcher.Start(
                Task.Retry(
                    cb => this.api.GetAllSlots(this.user.Namespace, this.user.UserId, 
        ...
        {   
            ...
            this.taskDispatcher.Start(
                Task.Retry(
                    cb => this.api.GetSlot(this.user.Namespace, this.user.UserId, 
                        this.user.AccessToken, slotId, result => cb(result)),
        ...
        public void CreateSlot(byte[] data, string dataName, ResultCallback<Slot> callback)
        {
            ...
            this.taskDispatcher.Start(
                Task.Retry(
                    cb => this.api.CreateSlot(this.user.Namespace, this.user.UserId, 
                        this.user.AccessToken, data, dataName, result => cb(result)),
                    result => this.coroutineRunner.Run(() => callback((Result<Slot>) result)),
                    this.user));
        }
        ...
        public void UpdateSlot(string slotId, byte[] data, string dataName, ResultCallback<Slot> callback)
        {
            ...
            this.taskDispatcher.Start(
                Task.Retry(
                    cb => this.api.UpdateSlot(this.user.Namespace, this.user.UserId, 
                        this.user.AccessToken, slotId, data, dataName, result => cb(result)),
                    result => this.coroutineRunner.Run(() => callback((Result<Slot>) result)),
                    this.user));
        }
        ...
        public void DeleteSlot(string slotId, ResultCallback callback)
        {
            ...
            this.taskDispatcher.Start(
                Task.Retry(
                    cb => this.api.DeleteSlot(this.user.Namespace, this.user.UserId, 
                        this.user.AccessToken, slotId, result => cb(result)),
