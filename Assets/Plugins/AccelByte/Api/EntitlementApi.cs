...
namespace AccelByte.Api
{
    ...
    internal class EntitlementApi
    {
        ...
        public IEnumerator<ITask> GetUserEntitlements(string @namespace, string userId,
            string userAccessToken, int page, int size,  ResultCallback<PagedEntitlements> callback)
        {
            ...
            Assert.IsNotNull(userAccessToken,
                "Can't get user entitlements! UserAccessToken parameter is null!");
            ...
            var result = response.TryParseJsonBody<PagedEntitlements>();
            callback.Try(result);
        }
    }
