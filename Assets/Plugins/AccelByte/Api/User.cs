    ...
    public class User
    {
        ...
        public void LoginWithUserName(string userName, string password, ResultCallback callback)
        {
            ...
            this.taskDispatcher.Start(
                Task.Retry(
                    cb => this.LoginWithUserNameAsync(userName, password, result => cb(result)),
        ...
        public void LoginWithOtherPlatform(PlatformType platformType, string platformToken, ResultCallback callback)
        {
            this.taskDispatcher.Start(
                Task.Retry(
                    cb => this.LoginWithOtherPlatformAsync(platformType, platformToken, result => cb(result)),
                    result => this.coroutineRunner.Run(() => callback((Result) result)),
                    null));
        }
        ...
        public void LoginWithLauncher(ResultCallback callback)
        {
            ...
            this.taskDispatcher.Start(
                Task.Retry(
                    cb => this.LoginWithLauncherAsync(authCode, result => cb(result)),
                    result => this.coroutineRunner.Run(() => callback((Result) result)),
                    null));
        }
        ...
        {
            var tokenRefreshBackoff = TimeSpan.FromSeconds(10);
            var rand = new Random();
            ...
            ResultCallback<UserData> callback)
        {
            ...
            this.taskDispatcher.Start(
                Task.Retry(
                    cb => this.RegisterAsync(userInfo, result => cb(result)),
                    result => this.coroutineRunner.Run(() => callback((Result<UserData>) result)),
                    null));
        }

        private IEnumerator<ITask> RegisterAsync(UserInfo userInfo, ResultCallback<UserData> callback)
        {
            Result<string> clientTokenResult = null;

            yield return Task.Await(this.GetClientToken(result => clientTokenResult = result));
            ...
            yield return Task.Await(this.userApi.Register(this.@namespace, clientTokenResult.Value, userInfo,
                callback));
        }
            ...
            ResultCallback<UserData> callback)
        {
            ...
            this.taskDispatcher.Start(
                Task.Retry(
                    cb => this.UpdateAsync(updateRequest, result => cb(result)),
                    result => this.coroutineRunner.Run(() => callback((Result<UserData>) result)),
                    this));
        }
            ...
            ResultCallback<UserData> callback)
        {
            ...
            {
                yield return Task.Await(this.ForceRefresh());
            }
            ...
            ResultCallback<UserData> callback)
        {
            ...
            this.taskDispatcher.Start(
                Task.Retry(
                    cb => this.UpgradeAsync(userName, password, result => cb(result)),
                    result => this.coroutineRunner.Run(() => callback((Result<UserData>) result)),
                    this));
        }
            ...
            string verificationCode, ResultCallback<UserData> callback)
        {
            ...
            this.taskDispatcher.Start(
                Task.Retry(
                    cb => this.UpgradeAndVerifyAsync(userName, password, verificationCode, 
                        result => cb(result)),
                    result => this.coroutineRunner.Run(() => callback((Result<UserData>) result)),
                    this));
        }
