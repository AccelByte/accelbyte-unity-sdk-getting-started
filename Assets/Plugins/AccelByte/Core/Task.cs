...
namespace AccelByte.Core
{
    public static class Task
    {
        ...
        public static ITask Await(IEnumerator<ITask> tasks)
        {
            return new SequenceTask(tasks);
        }

        public static ITask Delay(uint milliseconds)
        {
            return new DelayTask(milliseconds);
        }

        public static IEnumerator<ITask> Retry(Func<Action<IResult>, IEnumerator<ITask>> tasks,
            Action<IResult> callback, User user, 
            int initialDelay = 1000, int maxDelay = 30000, int totalTimeout = 60000)
        {
            ...
            do
            {
                ...
                switch (result.Error.Code)
                {
                    ...
                    case ErrorCode.GatewayTimeout:
                        yield return Delay((uint) (nextDelay + rand.Next(-nextDelay / 4, nextDelay / 4)));
